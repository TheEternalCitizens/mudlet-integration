"use strict";
var conn, output, input, debugtrack, gameCharacter;
var c = {};
//-----Protocol Code
...
//-----Interpreter Code
		...
		for (var i=0; i<lines.length; i++) {
			...
			if (str.substring(0,7)==='SECRET ') {
				...
				/*sendSys("USER " + userName + "\n" +
								"SECRET " + secret + "\n" +
								"HASH " + hash + "\n" +
								(window.charName && window.charName.length > 0 ? "CHAR " + window.charName + "\n" : "")
				);*/
				sendSys("USER " + userName);
				...
				if (window.location == "http://test.skotos.net/orchil/marrach/marrach.htm") {
					printScreened("Orchil Testing Note:  If you're on Marrach and not being fully logged in, try typing the name of a character on your account.");
				}
	...
	function loadCookie(name) {
		if (name=="user") {
				//return prompt("Username (whitelisted accounts only)");
		}
		if (name=="pass") {
		...
		//alert(allcokies);
		//alert(allcookies.substring(start,end));
